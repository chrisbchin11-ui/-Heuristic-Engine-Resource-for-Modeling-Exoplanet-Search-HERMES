...
def analyze_system(star_name):
    ...
    for planet in planets:
        ...
        hz_status = habitable_zone_check(planet['pl_orbper'], stellar_mass)
    ...
    for planet in planets:
        hz = habitable_zone_check(planet['pl_orbper'], stellar_mass)
...
def compare_systems(star_list):
    ...
    for star, planets in all_systems.items():
        ...
        if not planets:
            ...
            continue
        summary += f"\n{star} (stellar mass: {stellar_mass} solar masses): {len(planets)} confirmed planets\n"
        for planet in planets:
            hz = habitable_zone_check(planet['pl_orbper'], stellar_mass)

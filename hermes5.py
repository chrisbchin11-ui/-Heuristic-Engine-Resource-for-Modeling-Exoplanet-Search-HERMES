...
def get_stellar_temperature(star_name):
    url = "https://exoplanetarchive.ipac.caltech.edu/TAP/sync"
    params = {
        "query": f"select st_teff from ps where hostname like '{star_name}%'",
        "format": "json"
    }
    try:
        response = requests.get(url, params=params, timeout=30)
        data = response.json()
        for entry in data:
            if entry['st_teff'] is not None:
                return entry['st_teff']
        return 5778
    except Exception as e:
        print(f"Could not retrieve stellar temperature for {star_name}, defaulting to solar")
        return 5778
...
def habitable_zone_check(orbital_period_days, stellar_mass=1.0, stellar_temp=5778):
    ...
    # Calculate luminosity from stellar mass and temperature
    temp_ratio = stellar_temp / 5778
    luminosity = (stellar_mass ** 4) * (temp_ratio ** 4)
    
    # Kepler's Third Law — calculate orbital distance in AU
    distance_au = (period_years ** 2 * stellar_mass) ** (1/3)
    
    # Habitable zone boundaries scaled by luminosity
    inner_edge = 0.95 * (luminosity ** 0.5)
    outer_edge = 1.67 * (luminosity ** 0.5)
...
def analyze_system(star_name):
    ...
    stellar_temp = get_stellar_temperature(star_name)
    ...
    for planet in planets:
        ...
        hz_status = habitable_zone_check(planet['pl_orbper'], stellar_mass, stellar_temp)
        print(f"Habitable Zone Status: {hz_status}")
        print("---")
    
    summary = f"Star: {star_name}\nStellar Mass: {stellar_mass} solar masses\nStellar Temperature: {stellar_temp} K\nNumber of planets: {len(planets)}\n"
    for planet in planets:
        hz = habitable_zone_check(planet['pl_orbper'], stellar_mass, stellar_temp)
...
def compare_systems(star_list):
    ...
    for star in star_list:
        ...
        stellar_temp = get_stellar_temperature(star)
        all_systems[star] = planets
        stellar_masses[star] = stellar_mass
        stellar_temps[star] = stellar_temp
    ...
    for star, planets in all_systems.items():
        ...
        stellar_temp = stellar_temps[star]
        if not planets:
            summary += f"\n{star}: No confirmed planets found\n"
            continue
        summary += f"\n{star} (mass: {stellar_mass} solar masses, temp: {stellar_temp} K): {len(planets)} confirmed planets\n"
        for planet in planets:
            hz = habitable_zone_check(planet['pl_orbper'], stellar_mass, stellar_temp)

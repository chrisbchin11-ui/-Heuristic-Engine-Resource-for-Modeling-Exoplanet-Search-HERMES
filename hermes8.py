...
def plot_tess_lightcurve(tic_id, period=None, t0=None):
    ...
    if len(search_result) > 0:
        ...
        if period is not None and t0 is not None:
            # Check if t0 is in JD/BJD (usually millions) and convert to BTJD
            if t0 > 2400000:
                t0 = t0 - 2457000
                
            print(f"Folding and binning light curve on period: {period} days...")
            
            # 1. Fold the light curve using the period and correctly scaled transit midpoint
            folded_lc = lc.fold(period=period, epoch_time=t0)
            
            # 2. BIN THE DATA: Average the points into 15-minute chunks
            binned_lc = folded_lc.bin(time_bin_size=15/1440)
            
            # 3. Plot the raw, noisy data in the background (faint gray)
            ax = folded_lc.plot(linewidth=0, marker='.', color='gray', markersize=2, alpha=0.1)
            
            # 4. Plot the binned data on top (bright red)
            binned_lc.plot(ax=ax, linewidth=0, marker='o', color='red', markersize=4, alpha=1)
            
            ax.set_title(f"Binned & Folded TESS Light Curve: TIC {tic_id} (Period: {round(period, 2)}d)")
        else:
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            # 2. Pop up the graph (it will automatically fold if period/t0 are found!)
            plot_tess_lightcurve(star_id, period=period, t0=t0) 
            
            # 3. Once you close the graph window, run the AI analysis

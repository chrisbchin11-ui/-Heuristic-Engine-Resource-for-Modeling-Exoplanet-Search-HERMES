...
def refine_transit_model(binned_lc, period, t0):
    print("\nFitting physical transit model using BATMAN...")
    
    # Prepare time and flux arrays from the binned light curve
    t = binned_lc.time.value
    flux = binned_lc.flux.value
    
    # Set up BATMAN parameters
    params = batman.TransitParams()
    params.t0 = 0.0              # time of inferior conjunction (centered at phase 0)
    params.per = period          # orbital period
    params.rp = 0.1              # initial guess for planet-to-star radius ratio
    params.a = 15.0              # semi-major axis (in stellar radii)
    params.inc = 89.0            # orbital inclination (degrees)
    ...
    # Define a simple objective function to minimize (Chi-squared)
    def objective(x):
        params.rp = x[0]
        params.inc = x[1]
    ...
    # Define bounds so the optimizer cannot "cheat" and erase the planet
    # rp bounded between 0.5% and 20% of star size, inclination between 80 and 90 degrees
    bounds = ((0.005, 0.20), (80.0, 90.0))
    
    # Optimize using L-BFGS-B, which strictly respects the boundaries
    result = minimize(objective, [params.rp, params.inc], method='L-BFGS-B', bounds=bounds)
    
    optimized_rp, optimized_inc = result.x
    print(f"Refined Planet-to-Star Radius Ratio (Rp/R*): {optimized_rp:.4f}")
    print(f"Refined Orbital Inclination: {optimized_inc:.2f}°")
    
    # Generate the final best-fit model curve to plot
    params.rp = optimized_rp
    params.inc = optimized_inc
    fitted_flux = m.light_curve(params)
    
    return t, fitted_flux, optimized_rp, optimized_inc

def plot_tess_lightcurve(tic_id, period=None, t0=None):
    print(f"\nFetching light curve data for TIC {tic_id}...")
    search_result = lk.search_lightcurve(f"TIC {tic_id}", mission="TESS", author="SPOC")
    
    if len(search_result) > 0:
        lcfs = search_result.download_all(flux_column="pdcsap_flux")
        ...
        if period is not None and t0 is not None:
            ...
            print(f"Folding and binning light curve on period: {period} days...")
            
            # 1. Fold the light curve
            folded_lc = lc.fold(period=period, epoch_time=t0)
            
            # 2. BIN THE DATA
            binned_lc = folded_lc.bin(time_bin_size=15/1440)
            
            # 3. FIT THE TRANSIT MODEL (Notice it now catches 4 variables instead of 2)
            model_t, model_flux, opt_rp, opt_inc = refine_transit_model(binned_lc, period, t0)
            
            # 4. Plotting
            ax = folded_lc.plot(linewidth=0, marker='.', color='gray', markersize=2, alpha=0.1)
            binned_lc.plot(ax=ax, linewidth=0, marker='o', color='red', markersize=4, alpha=1)
            ax.plot(model_t, model_flux, color='blue', linewidth=2, label='BATMAN Best-Fit Model')
            
            ax.legend()
            ax.set_title(f"Refined Transit Model: TIC {tic_id} (Period: {round(period, 2)}d)")
            plt.show()
            
            # UPDATE: Return the parameters to the main loop so the LLM can use them
            return opt_rp, opt_inc
        else:
            print("No period data found. Plotting standard light curve...")
            ax = lc.plot(linewidth=0, marker='.', color='black', markersize=2, alpha=0.5)
            ax.set_title(f"TESS Light Curve: TIC {tic_id}")
            plt.show()
            return None, None
    else:
        print(f"No TESS light curve data found for TIC {tic_id}.")
        return None, None
...
def analyze_candidates(star_id, opt_rp=None, opt_inc=None):
    ...
    # Inject the HERMES-calculated physics into the LLM prompt!
    if opt_rp is not None and opt_inc is not None:
        summary += f"\n--- HERMES Derived Physical Parameters ---\n"
        summary += f"Calculated Planet-to-Star Radius Ratio (Rp/R*): {opt_rp:.4f}\n"
        summary += f"Calculated Orbital Inclination: {opt_inc:.2f} degrees\n\n"
...
1. FALSE POSITIVE PROBABILITY
   ...
   - How do the derived physical parameters (Rp/R* and inclination) support or challenge the planetary hypothesis?
...
def fetch_candidate_params(tic_id):
    print(f"Querying TOI database for TIC {tic_id} parameters...")
    url = "https://exoplanetarchive.ipac.caltech.edu/TAP/sync"
    
    # ADQL query to get the period and transit midpoint from the TOI table
    query = f"SELECT pl_orbper, pl_tranmid FROM toi WHERE tid = {tic_id}"
    ...
    try:
        ...
        if len(data) > 0:
            period = data[0].get("pl_orbper")
            t0 = data[0].get("pl_tranmid")
            return period, t0
    except Exception as e:
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            # 1. Fetch the candidate's period and transit midpoint
            period, t0 = fetch_candidate_params(star_id)
            
            # 2. Pop up the graph AND catch the physics variables!
            opt_rp, opt_inc = plot_tess_lightcurve(star_id, period=period, t0=t0) 
            
            # 3. Once you close the graph window, run the AI analysis, feeding it the newly calculated data
            analyze_candidates(star_id, opt_rp=opt_rp, opt_inc=opt_inc)
